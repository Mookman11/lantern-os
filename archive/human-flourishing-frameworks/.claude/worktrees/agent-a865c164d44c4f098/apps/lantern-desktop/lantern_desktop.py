...
class LanternChat(_BaseWindow):  # type: ignore[misc,valid-type]
            ...
            while True:
                ...
                if kind == "ready":
                    ...
                    if isinstance(value, str) and value.strip():
                        ...
                        if current:
                            self.input.insert(tk.END, " " + piece)
                        else:
                            self.input.insert("1.0", piece)
                        self.input.focus_set()
                        self._append_system(f"heard: \"{piece}\"")
                elif kind == "transcript":
                    self.mic_state = "idle"
                    self.mic_button.configure(text="Talk")
                    if isinstance(value, str) and value.strip():
                        self.input.focus_set()
                        self._append_system("Mic stopped — review and Send.")
                    else:
                        # Nothing partial arrived either; surface a hint.
                        current = self.input.get("1.0", tk.END).strip()
                        if not current:
    ...
    def _listen_thread(self, model_path: Path) -> None:
        ...
        sample_rate = 16000
        block_size = 4000
        # Voice-activity gate: feed Vosk only when amplitude crosses the gate,
        # with a hangover window so trailing low-energy syllables aren't clipped.
        rms_gate = 0.01
        hangover_seconds = 0.6
        block_seconds = block_size / float(sample_rate)
        hangover_blocks = max(1, int(round(hangover_seconds / block_seconds)))
        ...
        def callback(indata, frames, time_info, status):  # type: ignore[no-untyped-def]
            ...
            # indata is float32 in [-1, 1]; compute RMS for the VAD gate,
            # then convert to int16 PCM bytes for Vosk.
            mono = indata[:, 0]
            rms = float(np.sqrt(np.mean(np.square(mono, dtype=np.float64))))
            pcm16 = (mono * 32767).astype(np.int16).tobytes()
            audio_queue.put((pcm16, rms))
        ...
        try:
            with sd.InputStream(samplerate=sample_rate, channels=1, dtype="float32",
                ...
                while not self.mic_stop_event.is_set():
                    try:
                        chunk, rms = audio_queue.get(timeout=0.1)
                    except queue.Empty:
                        continue
                    if rms >= rms_gate:
                        ...
                        # Silence past the hangover window — skip feeding Vosk
                        # so it doesn't churn on background noise.
                        continue
                    if recognizer.AcceptWaveform(chunk):
                        utterance = json.loads(recognizer.Result()).get("text", "")
                        if utterance:
                            text_parts.append(utterance)
                            self.events.put(("partial_transcript", utterance))
            # drain any remaining and finalize
            while not audio_queue.empty():
                chunk, _rms = audio_queue.get_nowait()
                if recognizer.AcceptWaveform(chunk):
                    utterance = json.loads(recognizer.Result()).get("text", "")
                    if utterance:
                        text_parts.append(utterance)
