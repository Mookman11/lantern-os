...
# Papers — Papa @-referenced PDFs ingested into ~/.lantern/state/papers/ by
# a helper script. Lantern names the books, does not summarize them.
PAPERS_DIR = Path.home() / ".lantern" / "state" / "papers"
PAPERS_INDEX_PATH = PAPERS_DIR / "index.json"
_PAPERS_INDEX_CACHE: list[dict[str, Any]] | None = None
...
def load_papers_index() -> list[dict[str, Any]]:
    """Return the local papers index, cached after first read.

    Reads ~/.lantern/state/papers/index.json once and caches the parsed
    list of paper entries. Silent on missing or broken — returns [] so
    chat still works without any ingested papers.
    """
    global _PAPERS_INDEX_CACHE
    if _PAPERS_INDEX_CACHE is not None:
        return _PAPERS_INDEX_CACHE
    if not PAPERS_INDEX_PATH.exists():
        _PAPERS_INDEX_CACHE = []
        return _PAPERS_INDEX_CACHE
    try:
        data = json.loads(PAPERS_INDEX_PATH.read_text(encoding="utf-8"))
    ...
    for entry in papers:
        ...
        cleaned.append({
            ...
            "extracted_chars": entry.get("extracted_chars"),
        })
    _PAPERS_INDEX_CACHE = cleaned
    return _PAPERS_INDEX_CACHE
...
def classify_intent(message: str) -> str:
    ...
    if any(term in text for term in ("mask", "mode", "chameleon", "shapeshift", "imagination", "story", "comedian", "enigma", "converge")):
        return "hybrid"
    # Papers — Papa @-referenced PDFs ingested locally. Placed AFTER hybrid
    # so a bare "convergence" still fires hybrid; but a bare "papers", "pdf",
    # "book", "library", "singularity", or "asi" falls through to here.
    if any(term in text for term in ("paper", "papers", "pdf", "book", "books", "library", "singular", "singularity")):
        return "papers"
    if " asi " in f" {text} " or text.strip(" .!?,;:'\"") == "asi":
        return "papers"
...
def build_minimal_frame(message: str, intent: str, active_mode: str, repo_state: dict[str, Any], doctor: dict[str, Any] | None = None) -> dict[str, str]:
    ...
    if intent == "papers":
        ...
        n = len(papers)
        return {
            "Vibe": "Library mode — Lantern names the books, does not summarize them.",
            "Fact": f"{n} papers in the local library; titles named in Sources below.",
            "Boundary": "Lantern can name and quote excerpts but cannot replace reading.",
            "Next": "Say 'paper N' or part of a title to pull its first lines.",
        }
    if intent == "app":
        return {
