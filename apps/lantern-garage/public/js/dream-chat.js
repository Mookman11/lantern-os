  ...
  function sendMessage() {
    ...
    // !autowork <issue> [apply|commit|push] — observable autonomous work (epic #527 A2)
    // Streams the plan + diff + every step live. Dry run by default (nothing is
    // applied). apply = apply + tests, stop before commit. commit/push opt in.
    {
      const aw = text.match(/^!autowork(?:\s+#?(\d+))?(?:\s+(apply|commit|push))?\s*$/i);
      if (aw) {
        inputEl.value = "";
        inputEl.style.height = "auto";
        const issue = aw[1] ? parseInt(aw[1], 10) : null;
        const mode = (aw[2] || "dry").toLowerCase();
        if (!issue) {
          const u = document.createElement("div");
          u.className = "msg-row agent";
          u.innerHTML = `<div class="msg-label">Autonomous Workspace</div><div class="bubble">Usage: <code>!autowork &lt;issue#&gt; [apply|commit|push]</code><br><span style="opacity:0.7;font-size:12px">Default is a dry run — streams the plan and diff, applies nothing. <code>apply</code> applies + runs tests (stops before commit). <code>commit</code>/<code>push</code> opt into those steps.</span></div>`;
          messagesEl.appendChild(u); scrollToBottom();
          return;
        }
        const opts = { issue, dryRun: mode === "dry" };
        if (mode === "commit") opts.commit = true;
        if (mode === "push") opts.push = true;
        const modeLabel = opts.push ? "apply → commit → push → PR"
          : opts.commit ? "apply → tests → commit"
          : opts.dryRun ? "dry run · nothing applied"
          : "apply → tests · stop before commit";

        const row = document.createElement("div");
        row.className = "msg-row agent";
        row.innerHTML =
          `<div class="msg-label">Autonomous Workspace · #${issue}</div>` +
          `<div class="bubble" style="font-size:13px">` +
            `<div style="opacity:0.7;margin-bottom:6px">Mode: ${escapeHtml(modeLabel)}</div>` +
            `<div class="aw-steps" style="font-family:monospace;font-size:12px;line-height:1.7"></div>` +
            `<pre class="aw-diff" style="display:none;margin-top:8px;max-height:340px;overflow:auto;white-space:pre;background:#0a0e14;border:1px solid #1c2733;border-radius:6px;padding:10px;font-size:11.5px"></pre>` +
            `<div class="aw-done" style="margin-top:8px"></div>` +
          `</div>`;
        messagesEl.appendChild(row);
        scrollToBottom();
        const stepsEl = row.querySelector(".aw-steps");
        const diffEl = row.querySelector(".aw-diff");
        const doneEl = row.querySelector(".aw-done");
        const lines = {}; // phase -> element (so repeated phases update in place)

        const renderStep = (d) => {
          const icon = d.status === "done" ? "✓" : d.status === "error" ? "✗" : d.status === "skipped" ? "⊘" : "…";
          const color = d.status === "done" ? "#4caf82" : d.status === "error" ? "#e05555" : d.status === "skipped" ? "#888" : "#e2c97e";
          let el = lines[d.phase];
          if (!el) { el = document.createElement("div"); lines[d.phase] = el; stepsEl.appendChild(el); }
          let extra = "";
          if (d.title) extra = ` — ${escapeHtml(d.title)}`;
          else if (d.branch) extra = ` — ${escapeHtml(d.branch)}`;
          else if (d.reason) extra = ` (${escapeHtml(d.reason)})`;
          else if (d.error) extra = ` — ${escapeHtml(d.error)}`;
          el.innerHTML = `<span style="color:${color}">${icon}</span> ${escapeHtml(d.phase)}${extra}`;
        };

        const renderDiff = (diffText) => {
          diffEl.style.display = "block";
          diffEl.innerHTML = String(diffText || "(empty diff)").split("\n").map((l) => {
            const e = escapeHtml(l);
            if (l.startsWith("+") && !l.startsWith("+++")) return `<span style="color:#4caf82">${e}</span>`;
            if (l.startsWith("-") && !l.startsWith("---")) return `<span style="color:#e05555">${e}</span>`;
            if (l.startsWith("@@")) return `<span style="color:#6ab0f3">${e}</span>`;
            if (l.startsWith("diff ") || l.startsWith("index ") || l.startsWith("+++ ") || l.startsWith("--- ")) return `<span style="opacity:0.55">${e}</span>`;
            return e;
          }).join("\n");
        };

        isStreaming = true;
        fetch(`${serverBase}/api/convergence/autonomous-work/stream`, {
          method: "POST",
          ...
          for (;;) {
            ...
            for (const chunk of chunks) {
              ...
              if (ev === "step") renderStep(d);
              else if (ev === "diff") renderDiff(d.diffText);
              else if (ev === "error") doneEl.innerHTML = `<span style="color:#e05555">✗ ${escapeHtml(d.error || "error")}</span>`;
              else if (ev === "done") {
                const parts = [d.ok ? "✓ done" : "✗ stopped"];
                if (d.stoppedAt) parts.push(`stopped at ${d.stoppedAt}`);
                if (d.testsPassed != null) parts.push(`tests ${d.testsPassed ? "passed" : "failed"}`);
                parts.push(`applied ${d.applied ? "yes" : "no"}`, `committed ${d.committed ? "yes" : "no"}`, `pushed ${d.pushed ? "yes" : "no"}`);
                let html = `<div style="font-size:12px;opacity:0.85">${parts.map(escapeHtml).join(" · ")}</div>`;
                if (d.message) html += `<div style="font-size:12px;margin-top:4px">${escapeHtml(d.message)}</div>`;
                if (d.prUrl) html += `<a href="${escapeHtml(d.prUrl)}" target="_blank" class="suggestion" style="display:inline-block;margin-top:6px;border-color:#e2c97e;color:#e2c97e">Open PR ↗</a>`;
          ...
          doneEl.innerHTML = `<span style="color:#e05555">✗ ${escapeHtml(e.message)}</span>`;
        }).finally(() => { isStreaming = false; scrollToBottom(); });
        return;
      }
    }
